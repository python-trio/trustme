...
def _smells_like_pyopenssl(ctx):
    return ctx.__module__.startswith("OpenSSL")
...
class ServerCert(object):
    ...
    def use(self, ctx):
        if isinstance(ctx, ssl.SSLContext):
            ...
            key = load_privatekey(FILETYPE_PEM, self.private_key_pem)
            ctx.use_privatekey(key)
            # XX this will need to be changed somehow if we ever have
            # multi-cert chains
            cert = load_certificate(FILETYPE_PEM, self.cert_chain_pem)
